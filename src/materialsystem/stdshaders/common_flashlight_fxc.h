...
// Improved Subsurface Scattering for Flashlights
float3 DoFlashlightSSS(float3 lightDir, float3 normal, float3 viewDir, float thickness, float3 sssColor, float intensity)
{
	// Calculate backlit component (light coming from behind)
	float backlit = max(0.0, -dot(normal, lightDir));

	// Thickness creates transmission effect
	// Lower thickness values = more light transmission (thinner areas)
	float transmittance = pow(saturate(thickness), 2.0);
	backlit *= transmittance;

	// View angle affects SSS visibility
	float VdotL = dot(viewDir, -lightDir);
	float facingLightBackside = smoothstep(-0.5, 0.5, VdotL);
	backlit *= facingLightBackside;

	// Stronger intensity for flashlights
	return sssColor * backlit * intensity;
}
